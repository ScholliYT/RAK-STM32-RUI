...
static uint8_t *AckTimeoutRetries_buff;
static uint32_t AckTimeoutRetries_len;
static SERVICE_LORA_SEND_INFO AckTimeoutRetries_info;
...
int32_t service_lora_send(uint8_t *buff, uint32_t len, SERVICE_LORA_SEND_INFO info, bool blocking)
{
...
#elif defined(LORA_STACK_104)
        if (true == info.retry_valid)
        {
            AckTimeoutRetries = info.retry;
        }
        else
        {
            AckTimeoutRetries = service_lora_get_retry();
        }
#endif
    ...
    if(AckTimeoutRetriesCounter == 0)
    {

        AckTimeoutRetries_buff = buff;
        AckTimeoutRetries_len = len;
...
int32_t service_lora_set_IsCertPortOn(bool IsCertPortOn)
{
    if(IsCertPortOn)
        ...
        service_nvm_set_certi_to_nvm(0);
